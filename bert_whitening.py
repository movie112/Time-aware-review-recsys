...
ROOT =  ROOT =  '/home/yeonghwa/workspace/reviewgraph/final_c/'
...
parser.add_argument('--dataset_name', type=str, default=DATA_NAME,
                    help='dataset name')
parser.add_argument('--dataset_path', type=str, default=ROOT+'/dataset/{}/{}.json'.format(DATA_NAME, DATA_NAME),
...
args.dataset_path = ROOT+'/dataset/{}/{}.json'.format(DATA_NAME, DATA_NAME)
args.vec_dim = 64
...
def collate_fn(data):
    ...
    for i in range(len(tokens)):
        tokens_lst.append(bert_tokenizer.convert_tokens_to_string(tokens[i]))

    
    encoding = bert_tokenizer(tokens_lst, return_tensors='pt', padding=True,
