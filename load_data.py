...
def read_amazon_review_raw_data_and_split(dataset_path, word_dim=WORD_DIM):
    ...
    if os.path.exists(dataset_info_path):
        ...
        print('Start reading data to pandas.')

        data = None
        if 'yelp2013' in dataset_path:
            data = load_yelp2013(dataset_path)
            data['time'] = pd.to_datetime(data['time'], format='%Y-%m-%d %H:%M:%S').dt.strftime('%Y%m%d')
        else:
            ...
            # %m %d, %Y -> %Y-%m-%d
            data['time'] = pd.to_datetime(data['time'], format='%m %d, %Y').dt.strftime('%Y%m%d')
        print(data.iloc[0])
        ...
        print(f'Truncate review length to {review_length} words')
...
#                                     test_data.loc[test_drop_item_data_index]])
    train_data = pd.concat([valid_data.loc[valid_drop_user_data_index],
                                  valid_data.loc[valid_drop_item_data_index],
                                  test_data.loc[test_drop_user_data_index],
                                  test_data.loc[test_drop_item_data_index]], ignore_index=True) ###
...
def load_data_for_triplet(dataset_path):
    print('Start loading triplet data')
    dir_path, basename = get_dir_and_base_name(dataset_path)
...
####
def load_sentiment_data(dataset_path, word_dim=WORD_DIM):
    """
    :param dataset_path:
    :param word_dim
    """
    ...
    if os.path.exists(train_data_path):
        ...
        train_data = \
            train_data.loc[:, ['user_id', 'item_id', 'review_text', 'rating', 'time']]
        valid_data = \
            valid_data.loc[:, ['user_id', 'item_id', 'review_text', 'rating', 'time']]
        test_data = \
            test_data.loc[:, ['user_id', 'item_id', 'review_text', 'rating', 'time']]
...
def get_dir_and_base_name(file_path):
    ...
    # dir_path = 'D:/movie/reviewgraph/baseline/RGCL_try/dataset/Digital_Music_5'
    # basename = 'Digital_Music_5'
