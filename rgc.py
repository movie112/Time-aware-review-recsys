...
def config():
    ...
    parser.add_argument('--model_short_name', type=str, default='RGC')
    parser.add_argument('--logger', type=str, default='logger')
    parser.add_argument('--logger_file', type=str, default='')
    parser.add_argument('--root_path', type=str, default='D:/movie/reviewgraph/baseline/RGCL_final/')
...
class GCMCGraphConv(nn.Module, ABC):
    ...
    def forward(self, graph, feat, weight=None): ######  
        ...
        with graph.local_scope():
            ...
            # graph.edata['pa'] = torch.sigmoid(self.prob_score(graph.edata['attn']))
            # graph.edata['ra'] = torch.sigmoid(self.review_score(graph.edata['attn']))
...
class MLPPredictor(nn.Module, ABC):
    ...
    def apply_edges(self, edges):
        ...
        score = self.predictor(torch.cat([h_u, h_v], dim=1))
        return {'score': score}
...
def train(params):
    ...
    for iter_idx in range(1, params.train_max_iter):
        ...
        valid_mse = evaluate(args=params, net=net, dataset=dataset, segment='valid') ##

        # logging_str = f"Iter={iter_idx:>3d}, " \
        #               f"Train_RMSE={train_rmse:.4f}, Valid_RMSE={valid_rmse:.4f}, "
        logging_str = f"Iter={iter_idx:>3d}, " \
                      f"Train_MSE={train_mse:.4f}, Valid_MSE={valid_mse:.4f}, "
        ...
        if valid_mse < best_valid_mse:
            ...
            test_mse = evaluate(args=params, net=net, dataset=dataset, segment='test')
    ...
    logger.info(logging_str)
    print(f'Best Iter Idx={best_iter}, Best Valid MSE={best_valid_mse:.4f}, Best Test MSE={best_test_mse:.4f}')
    logger.info(f'Best Iter Idx={best_iter}, Best Valid MSE={best_valid_mse:.4f}, Best Test MSE={best_test_mse:.4f}')
...
if __name__ == '__main__':
    ...
    with open(ROOT+'log_rgc/'+'best.csv', 'a', newline='')  as f:
        w = csv.writer(f)
        w.writerow(best_lst)
