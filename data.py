...
class MovieLens(object):
    ...
    def __init__(self, dataset_name, dataset_path, device, review_fea_size, beta,
        ...
        self.u_attn_path = ROOT + f'dataset/{dataset_name}/u_attn{self.beta}.pickle'
        self.i_attn_path = ROOT + f'dataset/{dataset_name}/i_attn{self.beta}.pickle'
...
#####
    def _generate_enc_graph(self, rating_pairs, rating_values,
                            add_support=False):
        seq_model = gru_module(input_dim=INPUT_DIM, gru_dim=GRU_DIM, time_dim=TIME_DIM, beta=self.beta) # 64, 32, 32, 1.0
        seq_model.cuda()
        rating_row, rating_col = rating_pairs
        with open(self.u_attn_path, 'rb') as f:
            u_attn = pickle.load(f)
        with open(self.i_attn_path, 'rb') as f:
            i_attn = pickle.load(f)
        u_attn = tuple(u_attn)
        i_attn = tuple(i_attn)
        ...
        for rating in self.possible_rating_values:
            ...
            u_attn_dict[str(rating)] = torch.stack(u_attn)[ridx]
            i_attn_dict[str(rating)] = torch.stack(i_attn)[ridx]
        ...
        for rating in self.possible_rating_values:
            ...
            graph[str(rating)].edata['attn'] = u_attn_dict[str(rating)]
            graph['rev-%s' % str(rating)].edata['attn'] = i_attn_dict[str(rating)]
        # sanity check
